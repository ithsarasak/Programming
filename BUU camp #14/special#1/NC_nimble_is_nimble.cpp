...
long long n, m, ice[N][N], qm[N][N][N][N], mem[N][N][N][N];
...
{
    scanf("%lld %lld",&n,&m);
    for( int i = 1 ; i <= n ; i++ ) for( int j = 1 ; j <= m ; j++ ) scanf("%lld",&ice[i][j]);
    
    for( int i = 0 ; i < 42 ; i++ ) for( int j = 0 ; j < 42 ; j++ )
            for( int k = 0 ; k < 42 ; k++ ) for( int l = 0 ; l < 42 ; l++ ) qm[i][j][k][l] = 1, mem[i][j][k][l] = -1;

    for( int i = 1 ; i <= n ; i++ ) for( int j = 1 ; j <= m ; j++ )
            for( int k = i ; k <= n ; k++ ) for( int l = j ; l <= m ; l++ )
                    for( int a = i ; a <= k ; a++ ) for( int b = j ; b <= l ; b++ ) qm[i][j][k][l] *= ice[a][b];
