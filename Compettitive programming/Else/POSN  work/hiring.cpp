...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        printf("HeapQuals : ");
        for( int j = 0 ; j < heapQuals.size() ; j++ ){
            cout << heapQuals[j] << " ";
        }
        printf("\n");
        totalQual += c[i].q;
        double maxQual = w / c[i].r;
        cout << totalQual << " " << maxQual << endl;
        while (totalQual > maxQual)
        {
            cout << totalQual << " " << maxQual << " " << heapQuals[0] << endl;
        ...
        cout <<"num : " << num << " best : " << best << " cost :" << cost << " Bestcost :" << bestCost << " Bestindex" << bestIndex << endl;
    }
